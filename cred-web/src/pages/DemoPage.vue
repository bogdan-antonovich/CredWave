...
async function handleGenerate() {
    ...
    const result = restaurantNameSchema.safeParse({
        restaurantName: input.value.trim(),
    });
    if (!result.success) {
    ...
    if (demoStore.searchResults.length === 0) {
        ...
        validationError.value =
            "No restaurants found. Try a different name or spelling.";
        return;
    }
...
const showLoading = computed(
    () => step.value === "searching" || step.value === "generating",
);
const showDisambiguation = computed(() => step.value === "disambiguation");
const showResults = computed(
    () => step.value === "results" && demoStore.blocks.length > 0,
);
        ...
        <main class="pt-14 flex-1">
            ...
            <section class="pt-24 pb-8 px-6">
                <div class="max-w-[560px] mx-auto text-center">
                    <div
                        class="hero-enter inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-accent/8 text-accent text-xs font-semibold mb-6"
                    >
                    ...
                    <template v-if="showResults">
                        <h1
                            class="hero-enter hero-enter-delay-1 text-3xl md:text-4xl font-bold font-display tracking-tight text-text-primary"
                        >
                            {{ selectedRestaurantName }}
                        </h1>
                        <p
                            class="hero-enter hero-enter-delay-2 mt-3 text-text-secondary text-lg"
                        >
                            3 AI-written replies per review — pick a tone, copy,
                            and post on Google.
                        </p>
                    </template>
                    <template v-else>
                        <h1
                            class="hero-enter hero-enter-delay-1 text-3xl md:text-4xl font-bold font-display tracking-tight text-text-primary"
                        >
                            Type your restaurant name.<br />Get 3 AI replies per
                            review — instantly.
                        </h1>
                        <p
                            class="hero-enter hero-enter-delay-2 mt-3 text-text-secondary text-lg"
                        >
                            No signup. No credit card. Just paste your
                            restaurant name and see what CredWave can do.
                        </p>
                    </template>
                </div>
            </section>
                    ...
                    <div class="hero-enter hero-enter-delay-3">
                        <div class="flex flex-col sm:flex-row gap-2">
                            <div class="relative flex-1">
                                <Search
                                    class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted"
                                />
                                <input
                                    v-model="input"
                                    type="text"
            ...
            <section v-if="showLoading" class="pb-24 px-6">
                <div class="max-w-[400px] mx-auto text-center py-16">
                    <div class="flex items-center justify-center gap-2 mb-6">
                        <span
                            class="w-2.5 h-2.5 rounded-full bg-accent animate-bounce"
                    ...
                    <p
                        class="text-sm font-medium text-text-primary transition-all duration-300"
                    >
                        {{ currentLoadingMsg }}
                    </p>
            ...
            <section v-if="showDisambiguation" class="pb-24 px-6">
                <div class="max-w-[560px] mx-auto">
                    <div class="reveal text-center mb-6">
                        <p class="text-sm text-text-secondary">
                            ...
                            <span class="font-semibold text-text-primary">{{
                                demoStore.searchResults.length
                            }}</span>
                            restaurant{{
                                demoStore.searchResults.length !== 1 ? "s" : ""
                            }}
                            matching "<span
                    ...
                    <div class="reveal space-y-2">
                        ...
                        >
                            <div
                                class="w-10 h-10 rounded-xl bg-surface-warm flex items-center justify-center shrink-0 group-hover:bg-accent/5 transition-colors duration-200"
                            >
                                <MapPin
                                    class="w-4 h-4 text-text-muted group-hover:text-accent transition-colors duration-200"
                                />
                            </div>
                            <div class="flex-1 min-w-0">
                                ...
                                <div
                                    class="flex items-center flex-wrap gap-x-2 gap-y-0.5 mt-0.5"
                                >
                                    <template v-if="option.rating">
                                        <span
                                            class="flex items-center gap-0.5 text-xs text-text-muted"
                                        >
                                            <Star
                                                class="w-3 h-3 text-warning fill-warning"
                                            />
                                            {{ option.rating }}
                                        </span>
                                    </template>
                            ...
                            <ChevronRight
                                class="w-4 h-4 text-text-muted group-hover:text-accent shrink-0 transition-colors duration-200"
                            />
                        </button>
                    </div>
                    ...
                    <div class="text-center mt-6">
                        <button
                            class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-border bg-white text-sm font-medium text-text-secondary hover:border-accent/40 hover:text-accent transition-all duration-200"
                            @click="reset"
                        >
                            <Search class="w-3.5 h-3.5" />
                            Search a different restaurant
                        </button>
                    </div>
                </div>
            </section>
            ...
            <section v-if="showResults" class="pb-24 px-6">
                <div class="max-w-[800px] mx-auto">
                    <div
                        class="reveal flex items-center gap-4 p-4 rounded-xl bg-accent/5 border border-accent/10 mb-8"
                    >
                        <div class="flex -space-x-1 shrink-0">
                            <span
                                class="w-6 h-6 rounded-full bg-rose-100 flex items-center justify-center text-[9px] font-bold text-rose-500 ring-2 ring-white"
                                >E</span
                            >
                            <span
                                class="w-6 h-6 rounded-full bg-indigo-100 flex items-center justify-center text-[9px] font-bold text-accent ring-2 ring-white"
                                >P</span
                            >
                            <span
                                class="w-6 h-6 rounded-full bg-amber-100 flex items-center justify-center text-[9px] font-bold text-amber-600 ring-2 ring-white"
                                >C</span
                            >
                        </div>
                        <p class="text-sm text-text-secondary">
                            <span class="font-semibold text-text-primary"
                                >3 tones per review.</span
                            >
                            Switch between Empathetic, Professional, and Casual.
                            Copy the one you like and reply directly on Google.
                        </p>
                    </div>
                    ...
                    <div
                        class="reveal mt-12 p-10 rounded-2xl bg-surface-dark text-center"
                    >
                        <p class="text-xl font-bold font-display text-white">
                            What if every review got a response — without you
                            lifting a finger?
                        </p>
                        <p
                            class="mt-3 text-sm text-white/45 max-w-[440px] mx-auto"
                        >
                            CredWave monitors your Google reviews 24/7. Every
                            review, three responses, zero effort.
                        </p>
                        <RouterLink
                            to="/pricing"
            ...
            <section v-if="showEmpty" class="pb-24 px-6">
                <div class="max-w-[400px] mx-auto text-center py-16">
                    <p class="text-sm text-text-muted">
                        No unanswered reviews found for this restaurant.
                    </p>
                    <button
                        class="mt-4 text-sm text-accent font-medium hover:underline"
                        @click="reset"
                    >
