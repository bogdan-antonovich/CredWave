...
describe('/reviews route', () => {
  ...
  beforeEach(async () => {
    ...
    await sql`TRUNCATE responses, reviews, restaurants, users CASCADE`;
    ...
    const [restaurant] = await sql<{ id: number }[]>`
      INSERT INTO restaurants (name, additional_info, user_id, google_place_id)
      VALUES ('Pasta Place', 'Family friendly Italian', ${userId}, 'ChIJ_fake_place_id')
      RETURNING id
    `;
    ...
    const [review] = await sql<{ id: number }[]>`
      ...
      VALUES (${restaurantId}, 'Great food!', 5, 'Alice', 'google-review-abc', 'https://example.com/alice.jpg', NOW(), false)
      RETURNING id
    `;
    reviewId = review.id;
  ...
  describe('POST /reviews/:id/generate', () => {
    ...
    it('overwrites existing response on second generate call', async () => {
      ...
      const rows = await sql`SELECT * FROM responses WHERE review_id = ${reviewId}`;
  ...
  describe('POST /reviews/:id/reply', () => {
    ...
    it('persists replied flag, reply text and replied_at to the database', async () => {
      ...
      const [row] = await sql<{ replied: boolean; reply_text: string; replied_at: Date }[]>`
    ...
    it('returns success and replied_at on success', async () => {
      const { body } = (await request(server)
        .post(`/reviews/${reviewId}/reply`)
        .set('Authorization', `Bearer ${userId}`)
        .send({ text: 'Thank you!' })
        .expect(201)) as { body: { success: boolean; replied_at: string } };
